...
Implemented variant:
    KGW left-hash detector following the MarkLLM KGW structure:
    seeded PRF over vocabulary IDs, configurable f-scheme over previous token
    context, green/red partitioning, green-token counting, z-score, and p-value.

Compatibility note:
    This implementation is algorithmically aligned with the published
    Kirchenbauer/MarkLLM KGW family, but it uses a pure-Python deterministic
    permutation instead of PyTorch's `torch.randperm`. Compatibility is tied to
    configurations generated with this exact `kgw-python-left-v1` variant.
"""
...
KGW_REFERENCE_VERSION = "kgw-python-left-v1"

KGW_REFERENCE_LIMITATIONS = [
    ...
    "This pure-Python reference variant does not claim byte-for-byte compatibility with every KGW implementation.",
]
...
def classify_backend(tokenizer: "ProvenanceTokenizer") -> tuple[str, str]:
    ...
    if getattr(tokenizer, "backend", "simple") == "huggingface":
        return "reference", "kgw-reference"
    return "reference-adapted", "controlled-local-only"


@dataclass(frozen=True)
class KGWReferenceConfig:
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "KGWReferenceConfig":
    ...
    @classmethod
    def from_file(cls, path: str | Path) -> "KGWReferenceConfig":
...
class KGWReferenceScorer:
    """KGW math utilities independent from text tokenization."""

    def __init__(self, config: KGWReferenceConfig, *, vocab_size: int):
        if vocab_size < 2:
        ...
        self._prf = self._permutation(config.hash_key)
    ...
    def _permutation(self, seed: int) -> list[int]:
        values = list(range(self.vocab_size))
        random.Random(int(seed)).shuffle(values)
        return values


class KGWReferenceDetector(WatermarkDetector):
    name = "kgw"
    version = KGW_REFERENCE_VERSION

    def __init__(self, config: KGWReferenceConfig, tokenizer: ProvenanceTokenizer):
        self.config = config
        self.tokenizer = tokenizer
        self.scorer = KGWReferenceScorer(config, vocab_size=tokenizer.vocabulary_size)
    ...
    def detect(self, text: str) -> DetectionResult:
        ...
        if score.scored_token_count < self.config.min_scored_tokens:
            return DetectionResult(
                ...
                limitations=KGW_REFERENCE_LIMITATIONS,
            )
